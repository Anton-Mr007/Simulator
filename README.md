...
Используется `kinematics_ground_truth` из `getMultirotorState()` — истинная
кинематика симулятора (не оценка полётного контроллера). Запись IMU/GPS/GT —
в раздельных потоках с независимыми частотами.
