...
struct NavState
{
    ...
    // смещения гироскопов, оценённые по статическому участку (для hdg_true)
                 ...
                 NavState &st, data_io::NavLogger &log)
{
    ...
    if (i % SNS_DECIMATION == 0)
    {
        ...
        // курс по горизонтальной проекции угловой скорости Земли.
        // Показания должны быть свободны от смещений гироскопа, иначе
        // сигнал ω Земли (≈4e-5 рад/с) тонет в смещении (~1e-3 рад/с).
        // Смещения берутся из статического участка — они стабильны за полёт.
