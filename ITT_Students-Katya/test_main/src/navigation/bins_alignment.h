...
struct AlignmentResult
{
    ...
    Vector bg = {0.0, 0.0, 0.0}; // смещения гироскопов, оценённые по статике
};
...
inline AlignmentResult alignBins(const std::string &imu_path, const SnsScan &scan)
{
    ...
    // Смещения гироскопов по статике: средние показания за вычетом проекции
    // угловой скорости Земли. Эталонный курс ψ берётся из angle.dat — на
    // неподвижном участке он точен, и только с ним разделяются bg и ω Земли
    // (иначе курс из гирокомпаса сам содержит ошибку от bg).
