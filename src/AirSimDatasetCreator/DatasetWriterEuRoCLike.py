...
class EuRoCDatasetWriter:
    ...
    def write_camera_image(self, camera_name: str, timestamp_ns: int, image_rgb: np.ndarray, time_s: float):
        ...
        with open(csv_path, "a", newline="") as f:
            f.write(f"{time_s:.6f}\t{timestamp_ns}\t{filename}\n")

    def write_imu_row(self, timestamp_ns, wx, wy, wz, ax, ay, az, time_s):
        with open(self.imu_csv_path, "a", newline="") as f:
            f.write(f"{time_s:.6f}\t{timestamp_ns}\t{wx}\t{wy}\t{wz}\t{ax}\t{ay}\t{az}\n")

    def write_gps_row(self, timestamp_ns, latitude, longitude, altitude, vx, vy, vz, time_s):
        with open(self.gps_csv_path, "a", newline="") as f:
            f.write(f"{time_s:.6f}\t{timestamp_ns}\t{latitude}\t{longitude}\t{altitude}\t{vx}\t{vy}\t{vz}\n")

    def write_gt_row(self, timestamp_ns, px, py, pz, qw, qx, qy, qz, vx, vy, vz, time_s):
        with open(self.gt_csv_path, "a", newline="") as f:
            f.write(f"{time_s:.6f}\t{timestamp_ns}\t{px}\t{py}\t{pz}\t{qw}\t{qx}\t{qy}\t{qz}\t{vx}\t{vy}\t{vz}\n")

    def write_gt_euler_row(self, timestamp_ns, roll, pitch, yaw, time_s):
        with open(self.angle_csv_path, "a", newline="") as f:
            f.write(f"{time_s:.6f}\t{timestamp_ns}\t{roll:.10f}\t{pitch:.10f}\t{yaw:.10f}\n")
